...
def generate_launch_description():
  # 自动检测 CH343 设备号变化：选择 /dev/ttyCH343USB* 中“第一个可见”的端口
  # 这样你就不需要每次手动从 USB0 改到 USB1。
  detected_ports = []
  try:
    detected_ports = sorted(glob.glob('/dev/ttyCH343USB*'))
  except Exception:
    detected_ports = []

  default_port = '/dev/ttyCH343USB0'
  if detected_ports:
    # 过滤掉可能的异常项，只保留 ttyCH343USB<数字>
    ch_match = [p for p in detected_ports if re.match(r'^/dev/ttyCH343USB\d+$', p)]
    if ch_match:
      default_port = ch_match[0]
